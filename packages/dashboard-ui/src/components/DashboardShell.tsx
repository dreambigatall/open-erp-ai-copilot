...
import { WidgetGrid } from './WidgetGrid.js'
import { AddWidgetModal } from './AddWidgetModal.js'
import { SystemMetricsCard } from './SystemMetricsCard.js'
import { CopilotPanel } from '../copilot/CopilotPanel.js'
import { CopilotProvider } from '../copilot/CopilotContext.js'
...
export function DashboardShell({
  ...
  const [showModal, setShowModal] = useState(false)
  const [sidebarOpen, setSidebarOpen] = useState(true)
  const { connector } = useDashboard()
  const llmProvider = typeof provider === 'string' ? null : provider
  ...
  const shellContent = (
    <div className="flex h-screen bg-gray-50 font-sans">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        >
          <div>
            ...
            {provider && (
              <p className="text-xs text-gray-400 font-mono mt-0.5">
                {llmProvider
                  ? `AI: ${llmProvider.name} / ${llmProvider.model}`
                  : `AI: ${String(provider)}`}
              </p>
            )}
          </div>
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 bg-brand-500
        ...
        <main className="flex-1 overflow-auto p-6">
          {metricsUrl && <SystemMetricsCard metricsUrl={metricsUrl} />}
          <WidgetGrid />
          {showModal && (
            <AddWidgetModal
              onClose={() => {
      ...
      {/* Copilot panel — fixed to right edge */}
      {llmProvider && connector && (
        <CopilotPanel providerName={`${llmProvider.name} / ${llmProvider.model}`} />
      )}
    </div>
  )

  // Wrap in CopilotProvider only if we have a real provider + connector
  if (llmProvider && connector) {
    return (
      <CopilotProvider
        provider={llmProvider}
        connector={connector}
        pageContext={`${activeNav} page`}
      >
        {shellContent}
      </CopilotProvider>
    )
